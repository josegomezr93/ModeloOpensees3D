function [F, hfig] = plotOSDef(NodesFile,FrameFile,DispFile,FactorD,vista,axi)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%           RESULTADO GRÁFICO DE DEFORMADA OPENSEES
%
...
% DispFile: archivo de definición de desplazamientos nodales 
% FactorD: Factor de escala
% vista: ejemplot [0,90] (plano X-Y)
% axi: definición como vector Ej: [xmin xmax ymin ymax zmin zmax];
% Únicamente valido para estructuras en 3D con 6 grados de libertad

% Ejemplo:
% [F, hfig] = plotOSDef('Nodes.tcl','BeamElem.tcl','Outputs\Displacement\nodesdisp.out',0.5,[0,90],axi);


wf=pwd;
ndf=6;
%% Lectura de datos
% cd(strcat('./',OutputsFolder))
nodesdisp=load(DispFile);
d = nodesdisp(:,2:end)'; % Vector solución
cd(wf)
...
if exist('elemB')>0
% Plotea vigas
   EdB=extract(EdofB,d(:,i));
   Bdisp(ExB,EyB,EzB,EdB,[3 4 1],FactorD);
end
% title('3D Structure')
% strcat('Factor for displacements x',num2str(FactorD))
xmin=axi(1,1);
xmax=axi(1,2);
ymin=axi(1,3);
ymax=axi(1,4);
